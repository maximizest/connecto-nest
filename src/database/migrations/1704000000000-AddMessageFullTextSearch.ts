...
export class AddMessageFullTextSearch1704000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      ON messages USING GIN ((COALESCE(content, '') || ' ' || COALESCE(searchable_text, '')) gin_trgm_ops)
    `);

    // 검색 성능 향상을 위한 복합 인덱스
    await queryRunner.query(`
      CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_planet_created
      ON messages (planet_id, created_at DESC)
