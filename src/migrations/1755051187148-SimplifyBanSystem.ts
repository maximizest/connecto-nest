...
export class SimplifyBanSystem1755051187148 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "planet_users" DROP COLUMN "banExpiresAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "travel_users" DROP COLUMN "banExpiresAt"`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "users"."isBanned" IS '계정 정지 여부 (로그인 불가)'`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `COMMENT ON COLUMN "users"."isBanned" IS '계정 정지 여부'`,
    );
    await queryRunner.query(
      `ALTER TABLE "travel_users" ADD "banExpiresAt" TIMESTAMP`,
    );
    await queryRunner.query(
      `ALTER TABLE "planet_users" ADD "banExpiresAt" TIMESTAMP`,
    );
