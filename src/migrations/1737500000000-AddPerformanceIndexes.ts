...
export class AddPerformanceIndexes1737500000000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_travel_user_user_status"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_planet_user_user_status"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_travel_status_visibility"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_notification_user_status"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_message_planet_created"`,
    );
  }
}
