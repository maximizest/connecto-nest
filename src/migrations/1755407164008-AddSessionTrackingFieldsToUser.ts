...
export class AddSessionTrackingFieldsToUser1755407164008 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`COMMENT ON COLUMN "users"."bannedAt" IS '계정 정지 시간'`);
        await queryRunner.query(`ALTER TABLE "users" ADD "bannedReason" character varying(500)`);
        await queryRunner.query(`COMMENT ON COLUMN "users"."bannedReason" IS '계정 정지 사유'`);
        await queryRunner.query(`ALTER TABLE "users" ADD "bannedBy" integer`);
        await queryRunner.query(`COMMENT ON COLUMN "users"."bannedBy" IS '계정을 정지시킨 관리자 ID'`);
        await queryRunner.query(`ALTER TABLE "users" ADD "bannedUntil" TIMESTAMP`);
        await queryRunner.query(`COMMENT ON COLUMN "users"."bannedUntil" IS '계정 정지 해제 예정 시간'`);
        await queryRunner.query(`ALTER TABLE "users" ADD "lastForcedLogout" TIMESTAMP`);
        await queryRunner.query(`COMMENT ON COLUMN "users"."lastForcedLogout" IS '마지막 강제 로그아웃 시간'`);
        await queryRunner.query(`ALTER TABLE "users" ADD "sessionVersion" integer NOT NULL DEFAULT '0'`);
        await queryRunner.query(`COMMENT ON COLUMN "users"."sessionVersion" IS '세션 버전 (강제 로그아웃 시 증가)'`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`COMMENT ON COLUMN "users"."lastForcedLogout" IS '마지막 강제 로그아웃 시간'`);
        await queryRunner.query(`ALTER TABLE "users" DROP COLUMN "lastForcedLogout"`);
        await queryRunner.query(`COMMENT ON COLUMN "users"."bannedUntil" IS '계정 정지 해제 예정 시간'`);
