...
export class DistributedCacheService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    const { DistributedEventService } = await import(
      '../events/distributed-event.service'
    );
    this.distributedEventService = this.moduleRef.get(DistributedEventService, {
      strict: false,
    });
  }
  ...
  async handleCacheInvalidation(data: {
    ...
    this.logger.debug(
      `Cache invalidated by replica ${data.invalidatedBy}: ${data.patterns.join(', ')}`,
    );
  }
}
