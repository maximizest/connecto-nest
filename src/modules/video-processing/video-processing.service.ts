...
export class VideoProcessingService {
  ...
  async getProcessingStats(userId?: number): Promise<{
    ...
    const queryBuilder =
      this.videoProcessingRepository.createQueryBuilder('processing');

    if (userId) {
      queryBuilder.where('processing.userId = :userId', { userId });
    }
    ...
    const stats = jobs.reduce(
      (acc, job) => {
        ...
        acc.totalInputSize += job.inputFileSize;

        if (job.status === VideoProcessingStatus.COMPLETED) {
          if (job.outputTotalSize) {
            acc.totalOutputSize += job.outputTotalSize;
          }

          const compressionRatio = job.getCompressionRatio();
          if (compressionRatio !== null) {
            acc.compressionRatios.push(compressionRatio);
          }

          const processingTime = job.getProcessingDuration();
          if (processingTime) {
